            ...
            if (isFastSearch)
            {
                            ...
                            if (chapterBuffer.Length > 20)
                            {
                                ...
                                for (int j = 0; j < this.Chapters[book.Value.VersesInChapterStartIndex + i].Verses.Count; j++)
                                {
                                    ...
                                    if (verse.Length > 0 && verse.StartPos < chapterBuffer.Length)
                                    {
                                        ...
                                        var verseText = RawGenTextReader.CleanXml(this.Serial.IsIsoEncoding ? (new Latin1Encoding()).GetString(verseBytes, 0, verseBytes.Length) : Encoding.UTF8.GetString(verseBytes, 0, verseBytes.Length), true);
                ...
                foreach (var index in foundBitIndexes)
                {
                    ...
                    if (counter <= 200)
                    {
                        ...
                        Array.Copy(chapterBuffer, (int)verse.StartPos, verseBytes, 0, verse.Length);
                        var verseText =
                            RawGenTextReader.CleanXml(this.Serial.IsIsoEncoding ? (new Latin1Encoding()).GetString(verseBytes, 0, verseBytes.Length) : Encoding.UTF8.GetString(verseBytes, 0, verseBytes.Length), true);
            ...
            ShowProgress progress)
        {
            try
            {
                ...
                for (i = 0; i < chaptListToSearch.Count; i++)
                {
                    ...
                    string chapter = this.Serial.IsIsoEncoding ? (new Latin1Encoding()).GetString(chapterBuffer, 0, chapterBuffer.Length) : Encoding.UTF8.GetString(chapterBuffer, 0, chapterBuffer.Length);
...
namespace System.Text
{
    public class Latin1Encoding : Encoding
    {
        ...
        public override string GetString(byte[] bytes, int byteIndex, int byteCount)
        {
            ...
            var charCount = GetChars(bytes, byteIndex, byteCount, chars, 0);
