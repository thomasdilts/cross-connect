	...
	public class GermanStemmer
	{
		...
		private void Resubstitute( StringBuilder buffer )
		{
			for ( int c = 0; c < buffer.Length; c++ ) 
			{
				...
				{
					buffer[c] = 'c';
					buffer.Insert( c + 1, 'h'.ToString() );
				}
				else if ( buffer[c] == '%' ) 
				{
					buffer[c] = 'e';
                    buffer.Insert(c + 1, 'i'.ToString());
				}
				else if ( buffer[c] == '&' ) 
				{
					buffer[c] = 'i';
                    buffer.Insert(c + 1, 'e'.ToString());
				}
				else if ( buffer[c] == '#' ) 
				{
					buffer[c] = 'i';
                    buffer.Insert(c + 1, 'g'.ToString());
				}
				else if ( buffer[c] == '!' ) 
				{
					buffer[c] = 's';
                    buffer.Insert(c + 1, 't'.ToString());
				}
			}
		}
	}
