		...
		private void  AddField(Document doc, FieldInfo fi, bool binary, bool compressed, bool tokenize)
		{
			...
			else
			{
				...
				if (compressed)
				{
					...
					byte[] b = new byte[toRead];
					fieldsStream.ReadBytes(b, 0, b.Length);
					f = new Field(fi.name, false, System.Text.Encoding.GetEncoding("UTF-8").GetString(Uncompress(b),0,b.Length), store, index, termVector);
					f.SetOmitTermFreqAndPositions(fi.omitTermFreqAndPositions);
					f.SetOmitNorms(fi.omitNorms);
				}
				else
					...
					if (fieldsData == null)
					{
						...
						try
						{
							...
							if (isCompressed)
							{
                                ...
                                fieldsData = System.Text.Encoding.GetEncoding("UTF-8").GetString(Enclosing_Instance.Uncompress(b), 0, b.Length);
							}
							else
							{
								if (Enclosing_Instance.format >= FieldsWriter.FORMAT_VERSION_UTF8_LENGTH_IN_BYTES)
								{
                                    ...
                                    fieldsData = System.Text.Encoding.GetEncoding("UTF-8").GetString(bytes, 0, toRead);
								}
								else
								{
