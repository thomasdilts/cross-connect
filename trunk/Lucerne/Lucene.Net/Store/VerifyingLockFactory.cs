...
namespace Lucene.Net.Store
{
	...
	public class VerifyingLockFactory:LockFactory
	{
		...
		private class CheckedLock:Lock
		{
			...
			{
				try
				{/* thomas dilts this cannot be done in WP7
                    System.Net.WebClient s = new System.Net.WebClient();
                    s.BaseAddress = Enclosing_Instance.host;
					s.OpenWriteAsync(new Uri(Enclosing_Instance.host));
                    s.UploadStringAsync(new Uri(Enclosing_Instance.host), Enclosing_Instance.id.ToString());
                    s.UploadStringAsync(new Uri(Enclosing_Instance.host), message.ToString());

                    s.DownloadStringAsync(new Uri(Enclosing_Instance.host));
                    s.
