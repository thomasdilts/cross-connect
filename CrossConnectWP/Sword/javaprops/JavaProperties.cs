...
namespace Sword.javaprops
{
    ...
    public class JavaProperties : Dictionary<string, string>
    {
        ...
        ///     if isLangDependentResource is false, this is the complete name of the resource.
        ///     Otherwise, this is just the name without the suffixes
        /// </param>
                ...
                using (
                    Stream stream =
                        assem.GetManifestResourceStream(
                            "Sword.Properties." + resourceName + "_" + isocode + ".properties"))
                {
                    if (stream != null)
                    {
                        reader.Parse(stream);
        ...
        /// </summary>
        /// <param name="defaults">
        ///     A Dictionary that holds a set of defafult key value pairs to
        ...
        public string GetProperty(string key)
        {
            ...
            if (objectValue != null)
            {
                return this.AsString(objectValue);
            }
            if (this.Defaults != null)
            {
                return this.AsString(this.Defaults[key]);
            }
        ...
        public IEnumerator PropertyNames()
        {
            ...
            if (this.Defaults != null)
            {
                combined = new Dictionary<string, string>(this.Defaults);

                for (IEnumerator e = this.Keys.GetEnumerator(); e.MoveNext();)
                {
                    string key = this.AsString(e.Current);
        ...
        {
            string oldValue = this.AsString(this[key]);
            this[key] = newValue;
            return oldValue;
        }
