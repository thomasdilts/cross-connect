    ...
    public class DictionaryZldDefReader : DictionaryRawDefReader
    {
        ...
        protected override async Task<byte[]> GetChapterBytes()
        {
            ...
            try
            {
                ...
                var chapterdata = new byte[entry.Length];
                fs.Read(chapterdata, 0, (int)entry.Length);
                fs.Dispose();
                // create a memorystream from the data
                MemoryStream memoryStream = new MemoryStream(chapterdata);

                // unzip the section
                ZlibStream zipStream;
                zipStream = new ZlibStream(memoryStream, CompressionMode.Decompress);
                var expandedBuffer = new byte[0];
                var buffer = new byte[10000];
                var length = 0;
                while ((length = zipStream.Read(buffer,0,10000)) > 0 )
                {
