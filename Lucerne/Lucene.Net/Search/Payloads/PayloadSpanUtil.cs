		...
		private void  QueryToSpanQuery(Query query, ICollection<byte[]> payloads)
		{
			...
			else if (query is MultiPhraseQuery)
			{
				...
				if (positions.Length > 0)
				{
					...
					System.Collections.Generic.List<object>[] disjunctLists = new System.Collections.Generic.List<object>[maxPosition + 1];
					...
					for (int i = 0; i < termArrays.Count; ++i)
					{
						...
						if (disjuncts == null)
						{
							disjuncts = (disjunctLists[positions[i]] = new System.Collections.Generic.List<object>(termArray.Length));
							++distinctPositions;
						}
					...
					{
						System.Collections.Generic.List<object> disjuncts = disjunctLists[i];
						if (disjuncts != null)
						{
