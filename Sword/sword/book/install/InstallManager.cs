...
namespace book.install
{
    ...
    public class WebInstaller
    {
        ...
        private void download(Uri source)
        {
            try
            {
                //source = new Uri("http://www.lazy8.nu/backup/BibleOsisToEpub.zip");
    ...
    public class SwordBook
    {
        ...
        public SwordBook(string modName)
        {
            isLoaded = false;
            IsolatedStorageFile root = System.IO.IsolatedStorage.IsolatedStorageFile.GetUserStoreForApplication();
            IsolatedStorageFileStream stream = root.OpenFile("mods.d/" + modName.ToLower() + ".conf", FileMode.Open);
            if (stream != null)
            {
                sbmd = new SwordBookMetaData(stream, modName);
                isLoaded = true;
            }
